...
In this tutorial, we're going to take you through deploying Zend Framework v1.11 to [the CloudKilat platform](http://www.cloudcontrol.com).
...
When using CloudKilat, we need to do this in a slightly different way. It involves, for each environment, setting an environment variable with the ironapp config add-on, then reading that information from the environment at runtime. We'll come to this shortly.
...
Now, we need to make our first deployment of both branches to the CloudKilat platform. To do this we checkout the master branch, create the application in our CloudKilat account and push and deploy both deployments. By running the following commands, this will all be done:
    ...
    ironapp cloudcontroldlzf create php

    // deploy the default branch
    ironapp cloudcontroldlzf/default push
    ironapp cloudcontroldlzf/default deploy

    // deploy the testing branch
    ironapp cloudcontroldlzf/testing push
    ironapp cloudcontroldlzf/testing deploy
...
You'll see output similar to the following:
    ...
    To ssh://cloudcontroldlzf@kilatiron.net/repository.git
...
Now let's be sure that everything is in order by having a look at the add-on configuration output, in this case for testing. To do that, run the command below:
    ...
    ironapp cloudcontroldlzf/default addon.add mysqls.free

    // Retrieve the settings
    ironapp cloudcontroldlzf/default addon mysqls.free

    // Initialise the mysqls.free addon for the testing deployment
    ironapp cloudcontroldlzf/testing addon.add mysqls.free

    // Retrieve the settings
    ironapp cloudcontroldlzf/testing addon mysqls.free
...
Now we need to configure the config add-on and store the respective environment setting in it. So run the following commands to do this:
    ...
    ironapp cloudcontroldlzf/default config.add APPLICATION_ENV=production

    // Set the testing environment setting
    ironapp cloudcontroldlzf/testing config.add APPLICATION_ENV=testing
...
####7.1.1 Deployment
    ...
    ironapp cloudcontroldlzf/default log deploy

####7.1.1 Errors

    ironapp cloudcontroldlzf/default log error
