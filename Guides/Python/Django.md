...
CloudKilat uses a [Procfile] to know how to start your processes. The example
...
database on CloudKilat because the filesystem is
...
application and create it on the CloudKilat platform: 
...
~~~bash
$ ironapp APP_NAME addon.add mysqls.free
...
~~~bash
$ ironapp APP_NAME run "python manage.py syncdb"
~~~

You can login to the admin console at `APP_NAME.kilatiron.net/admin`,
create some polls and see them at `APP_NAME.kilatiron.net/polls`.
