...
~~~bash
$ ironapp APP_NAME/default addon.add cloudamqp.lemur
~~~
...
~~~bash
$ ironapp APP_NAME/default addon.add config.free --SET_ENV_VARS --FLOWER_AUTH_EMAIL=YOUR_EMAIL_HERE
...
At this point you can see web interface at `http://APP_NAME.kilatiron.net`. But it hasn't got any workers yet.

## Scaling Celery Workers
Scaling Celery workers on CloudKilat is easy enough luckily. We have already defined how to run one in the `Procfile` earlier. So we can now go ahead and start the first one.

### Adding Workers

~~~bash
$ ironapp APP_NAME/default worker.add worker
# you can always list running workers like this
$ ironapp APP_NAME/default worker
# and also check the worker's log output with
$ ironapp APP_NAME/default log worker
...
If you refresh the web interface at `http://APP_NAME.kilatiron.net` you should be able to see the worker now.
...
$ ironapp APP_NAME/default worker
$ ironapp APP_NAME/default worker.remove WRK_ID
~~~
...
To run Celery commands use the ironapp run command. It will launch an additional container and connect you via SSH. You can then use the Celery commands in the an identical environment as the web interface and the workers itself.

~~~bash
$ ironapp APP_NAME/default run bash
Connecting...
...
This guide showed how to run both Flower aswell as a Celery worker on CloudKilat by specifying the commands in the `Procfile` and how to connect to a AMQP broker provided by the CloudAMQP Add-on with the credentials provided in the app's runtime environment. Additionally we learned how we can use the ironapp run command to use the Celery command line tool.
