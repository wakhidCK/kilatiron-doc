...
By default, Rails 3 uses SQLite for all the environments. However, it is not recommended to use SQLite on CloudKilat because the filesystem is [not persistent][filesystem]. To use a database, you should choose an Add-on from [Data Storage category][data-storage-addons].
...
~~~bash
$ ironapp APP_NAME/default addon.add elephantsql.turtle
...
~~~bash
$ ironapp APP_NAME/default run "rake db:migrate"
~~~

Congratulations, you can now access the app at http://APP_NAME.kilatiron.net.
