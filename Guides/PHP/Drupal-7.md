...
In this tutorial, we're going to take you through deploying Drupal 7 to [the CloudKilat platform](http://www.cloudcontrol.com).
...
Now, we need to make our first deployment of both branches to the CloudKilat platform. To do this we checkout the master branch, create the application in our CloudKilat account and *push* and *deploy* both deployments. By running the following commands, this will all be done:
    ...
    ironapp cloudcontroldldrupal create php
    
    // deploy the default branch
    ironapp cloudcontroldldrupal/default push    
    ironapp cloudcontroldldrupal/default deploy
    
    // deploy the testing branch
    ironapp cloudcontroldldrupal/testing push    
    ironapp cloudcontroldldrupal/testing deploy
...
To initialise mysqls, run the following commands and make a note of the output:
    ...
    ironapp cloudcontroldldrupal/default addon.add mysql.free

    // Retrieve the settings
    ironapp cloudcontroldldrupal/default addon mysql.free

    // Initialise the mysqls.free addon for the testing deployment
    ironapp cloudcontroldldrupal/testing addon.add mysql.free

    // Retrieve the settings
    ironapp cloudcontroldldrupal/testing addon mysql.free
...
Now we need to configure the config addon and store the respective environment setting in it. So run the following commands to do this:
    ...
    ironapp cloudcontroldldrupal/default config.add APPLICATION_ENV=production

    // Set the testing environment setting
    ironapp cloudcontroldldrupal/testing config.add APPLICATION_ENV=testing
...
Now that that's done, commit the changes we made earlier and push and deploy both environments again so that the new information will be used. This can be done quickly with the following commands:
    ...
    // deploy the default branch
    ironapp cloudcontroldlDrupal 7/default push    
    ironapp cloudcontroldlDrupal 7/default deploy
    
    git checkout testing
    git merge master
    
    // deploy the testing branch
    ironapp cloudcontroldlDrupal 7/testing push    
    ironapp cloudcontroldlDrupal 7/testing deploy
