...
In this tutorial, we're going to take you through deploying Kohana 3.2.0 to [the CloudKilat platform](http://www.cloudcontrol.com). If you need further information about Kohana, check out [the online user guide](http://kohanaframework.org/documentation) or jump in to [the IRC channel](irc://irc.freenode.net/kohana). Otherwise, let's get started.
...
Now, we need to make our first deployment of both branches to the CloudKilat platform. To do this we checkout the master branch, create the application in our CloudKilat account and push and deploy both deployments. By running the following commands, this will all be done:
    ...
    ironapp cloudcontroldlkohana create php

    // deploy the default branch
    ironapp cloudcontroldlkohana/default push
    ironapp cloudcontroldlkohana/default deploy

    // deploy the testing branch
    ironapp cloudcontroldlkohana/testing push
    ironapp cloudcontroldlkohana/testing deploy
...
When you do this, you'll see output similar to the following:
    ...
    To ssh://cloudcontroldlkohana@kilatiron.net/repository.git
...
Now that that's done, we need to configure two addons, config and mysqls. The config addon's required for determining the active environment and mysqls for storing our session information. To initialise these, run the following commands and make a note of the output:
    ...
    ironapp cloudcontroldlkohana/default addon.add mysqls.free

    // Initialise the mysqls.free addon for the testing deployment
    ironapp cloudcontroldlkohana/testing addon.add mysqls.free
...
Now we need to configure the config addon and store the respective environment setting in it. So run the following commands to do this:
    ...
    ironapp cloudcontroldlkohana/default config.add APPLICATION_ENV=production

    // Add the APPLICATION_ENV variable to testing
    ironapp cloudcontroldlkohana/testing config.add APPLICATION_ENV=testing
...
Now let's be sure that everything is in order by having a look at the add-on configuration output, in this case for testing. To do that, run the command below:
    ...
    ironapp cloudcontroldlkohana/testing addon mysqls.free
...
With that, you should be up and running, ready to create your next, amazing, PHP web application, using Kohana and CloudKilat. If you have any issues, feel free to email [support@cloudcontrol.com](mailto:support@cloudcontrol.com).
