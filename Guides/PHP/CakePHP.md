...
In this tutorial, we're going to take you through deploying CakePHP v2.2.1 to [the CloudKilat platform](http://www.cloudcontrol.com). 
...
Now, we need to make our first deployment of both branches to the CloudKilat platform. To do this we checkout the master branch, create the application in our CloudKilat account and push and deploy both deployments. 
By running the following commands, this will all be done:
    ...
    ironapp cloudcontrolledcakephp create php
    
    // deploy the default branch
    ironapp cloudcontrolledcakephp/default push    
    ironapp cloudcontrolledcakephp/default deploy 
    
    // deploy the testing branch
    ironapp cloudcontrolledcakephp/testing push    
    ironapp cloudcontrolledcakephp/testing deploy 
...
Now let's be sure that everything is in order by having a look at the add-on configuration output, in this case for testing. To do that, run the command below:
    ...
    ironapp cloudcontrolledcakephp/default addon.add mysql.free
    
    // Retrieve the settings
    ironapp cloudcontrolledcakephp/default addon mysql.free

    // Initialise the mysqls.free addon for the testing deployment
    ironapp cloudcontrolledcakephp/testing addon.add mysql.free
    
    // Retrieve the settings
    ironapp cloudcontrolledcakephp/testing addon mysql.free
...
Now we need to configure the config add-on and store the respective environment setting in it. So run the following commands to do this:
    ...
    ironapp cloudcontrolledcakephp/default config.add CAKE_ENV=production

    // Set the testing environment setting    
    ironapp cloudcontrolledcakephp/testing config.add CAKE_ENV=testing
...
With that, you should be up and running, ready to create your next, amazing, PHP web application, using CakePHP. If you want to save yourself some time, you can clone a copy of the modified CakePHP source from the CloudKilat Github repository. If you have any issues, feel free to email [support@cloudcontrol.com](mailto:support@cloudcontrol.com).
