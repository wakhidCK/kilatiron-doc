...
In this tutorial, we're going to take you through deploying Joomla v2.5 to [the CloudKilat platform](http://www.cloudcontrol.com). 
...
Now, we need to make our first deployment of both branches to the CloudKilat platform. To do this we checkout the master branch, create the application in our CloudKilat account and push and deploy both deployments. By running the following commands, this will all be done:
    ...
    ironapp cloudcontroldljoomla create php

    // deploy the default branch
    ironapp cloudcontroldljoomla/default push
    ironapp cloudcontroldljoomla/default deploy

    // deploy the testing branch
    ironapp cloudcontroldljoomla/testing push
    ironapp cloudcontroldljoomla/testing deploy
...
Now let's be sure that everything is in order by having a look at the add-on configuration output, in this case for testing. To do that, run the command below:
    ...
    ironapp cloudcontroldljoomla/default addon.add mysql.free

    // Retrieve the settings
    ironapp cloudcontroldljoomla/default addon mysql.free

    // Initialise the mysqls.free addon for the testing deployment
    ironapp cloudcontroldljoomla/testing addon.add mysql.free

    // Retrieve the settings
    ironapp cloudcontroldljoomla/testing addon mysql.free
...
Now we need to configure the config add-on and store the respective environment setting in it. So run the following commands to do this:
    ...
    ironapp cloudcontroldljoomla/default config.add APPLICATION_ENV=production

    // Set the testing environment settings
    ironapp cloudcontroldljoomla/testing config.add APPLICATION_ENV=testing
...
####7.1.1 Deployment
    ...
    ironapp cloudcontroldljoomla/default log deploy

####7.1.1 Errors

    ironapp cloudcontroldljoomla/default log error
