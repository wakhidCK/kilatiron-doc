...
Now, we need to make our first deployment of both branches to the CloudKilat platform. To do this we checkout the master branch, create the application in our CloudKilat account and push and deploy both deployments. By running the following commands, this will all be done:
    ...
    ironapp cloudcontroldlsymfony create php

    // deploy the default branch
    ironapp cloudcontroldlsymfony/default push
    ironapp cloudcontroldlsymfony/default deploy

    // deploy the testing branch
    ironapp cloudcontroldlsymfony/testing push
    ironapp cloudcontroldlsymfony/testing deploy
...
When you do this, you'll see output similar to the following:
    ...
    To ssh://cloudcontroldlsymfony@kilatiron.net/repository.git
...
To initialise mysqls, run the following commands and make a note of the output:
    ...
    ironapp cloudcontroldlsymfony/default addon.add mysql.free

    // Retrieve the settings
    ironapp cloudcontroldlsymfony/default addon mysql.free

    // Initialise the mysqls.free addon for the testing deployment
    ironapp cloudcontroldlsymfony/testing addon.add mysql.free

    // Retrieve the settings
    ironapp cloudcontroldlsymfony/testing addon mysql.free
...
Now we need to configure the config addon and store the respective environment setting in it. So run the following commands to do this:
    ...
    ironapp     cloudcontroldlsymfony/default config.add APPLICATION_ENV=production

    // Set the testing environment setting
    ironapp     cloudcontroldlsymfony/testing config.add APPLICATION_ENV=testing
...
Under ``apps/frontend/config`` open the file ``factories.yaml``. In that file, we need to adding in session configuration for both prod and test. These are identified by ``prod:`` and ``test:`` respectively. For each one, add in a configuration similar to that below, changing the respective details where necessary:
      ...
      storage:
        ...
        param:
          ...
          session_cookie_path: /
          session_cookie_domain: cloudcontroldlsymfony.kilatiron.net
